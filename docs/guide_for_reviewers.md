...
This document describes a checklist suitable as a guide for reviewers of pull requests against the Earth Data's tools repo. It is a guide only and reviewer discretion is still advised.
...
* [ ] Is this tool appropriate for the Earth Data repo (see [CONTRIBUTING.md](https://github.com/earth-data-community/galaxy-tools/blob/master/CONTRIBUTING.md))
...
* [ ] Is the owner set to `climate`? If this is an existing tool being migrated to Earth Data Community galaxy-tools, the previous `owner` can be maintained in order to preserve the tool lineage, but the owner need to grant authority to make changes to `iuc` in the main Tool Shed repository page and add `Intergalactic Utilities Commission` to the admin groups in "Manager repository administrators" **before** the PR is merged.
